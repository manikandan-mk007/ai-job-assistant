...
async function requestJobDataFromPage() {
  ...
  if (!tab?.id || !tab.url) {
    throw new Error("No active job tab found.");
  }
  ...
  const allowed =
    ...
    tab.url.includes("glassdoor.");

  if (!allowed) {
    throw new Error("Please open a LinkedIn, Indeed, Naukri, or Glassdoor job page first.");
  }
  ...
  if (!response || !response.description || response.description.length < 40) {
    throw new Error("Could not extract job description. Manually click LinkedIn '... more', then try again.");
  }
...
async function openCoverLetter() {
  ...
  if (currentSavedJobId) {
    ...
    showError("Please save the job first, then open cover letter.");
  }
}
...
function bindEvents() {
  ...
  els.savedBtn.addEventListener("click", () => {
    chrome.runtime.sendMessage({ type: "OPEN_SAVED" });
  });

  els.resumeBtn.addEventListener("click", () => {
    chrome.runtime.sendMessage({ type: "OPEN_RESUME" });
  });
}
